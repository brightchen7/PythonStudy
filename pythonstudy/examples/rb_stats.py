...
def load_rb(rb_path, date):
    ...
    df_rb = pd.read_excel(rb_path.joinpath(file_name_rb), sheet_name="持仓明细", header=1)
    df_rb_spot = (
        df_rb[["标的指数", "证券代码", "当前市值"]].rename(columns={"标的指数": "project"}).copy()
    )
    df_rb_future = (
        df_rb[["项目", "合约代码", "当前市值.1"]]
        .rename(columns={"项目": "project", "当前市值.1": "当前市值"})
        .copy()
    )
...
def market_value(df_rb, proj, proj_like=None):
    if proj is not None:
        ...
        return df_rb[
            df_rb["project"].apply(lambda x: re.match(proj_like, x) is not None)
        ]["当前市值"].sum()


# task1
def gen_arb_stats(df_rb):
    PB = market_value(df_rb, "PBRISK-BLK", None) + market_value(
        df_rb, "PBRISK-BLK300", None
    )
    FT = market_value(df_rb, "HKSL-BLK", None) + market_value(
        df_rb, "HKSL-BLK300", None
    )
...
def strategy(df_rb):
    ...
    edspbprop2_hk = market_value(df_rb[df_rb["当前市值"] < 0], "EDSPBPROP2-HK", None)
    edspbprop2_eln = market_value(df_rb[df_rb["当前市值"] < 0], "EDSPBPROP2-ELN", None)
    edspbprop2_pb = market_value(df_rb[df_rb["当前市值"] < 0], "EDSPBPROP2-PB", None)
...
def gen_sbl_stats(df_rb):
    ...
    sbl_zrt_mv = market_value(
        df_rb[df_rb["当前市值"] < 0], None, "^SBL-.*-ZRT$"
    ) + market_value(df_rb[df_rb["当前市值"] < 0], None, "^SBL-.*-HX$")
...
def lend(df_rb):
    ...
    HKSL_ = market_value(df_rb, None, "^HKSL-") - market_value(df_rb, None, "^HKSL-BLK")
    FT = HKSL + HKSL_
    PBSL = market_value(df_rb, "PBSL", None)
    PBSL_ = market_value(df_rb, None, "^PBSL-") - market_value(df_rb, None, "^PBSL-BLK")
    PB = PBSL + PBSL_
    summary = FT + PB
    print("##### Lend Complete #####")
    return {
...
def gen_result(path_txt, date, res_ls):
    def dict_to_df(res):
        KEY = list(res.keys())[0]
        df = pd.DataFrame(
            [[date, KEY, key, value] for key, value in res[KEY].items()],
